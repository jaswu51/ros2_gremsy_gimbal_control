...
public:
    IRImageNode() : Node("ir_image_node") {
        // 声明并获取参数
        this->declare_parameter("save_path", "src/ir_image/assets");
        ...
        timer_ = this->create_wall_timer(
            ...
            std::bind(&IRImageNode::timer_callback, this));

        RCLCPP_INFO(this->get_logger(), "IRImageNode initialized at 10Hz");
...
private:
    ...
    void initializePayloadSDK() {
        try {
            ...
            my_payload_->regPayloadStatusChanged(
                std::bind(&IRImageNode::onPayloadStatusChanged, this, 
                std::placeholders::_1, std::placeholders::_2));
            
            capture_state_ = CHECK_STORAGE;
            RCLCPP_INFO(this->get_logger(), "PayloadSDK initialized successfully in IR mode");
    ...
    void downloadAndPublishLatestImage() {
        ...
        latest_image_path_ = save_path_ + "/latest_ir_image.jpg";
        ...
        if (downloadImage(url, latest_image_path_)) {
            try {
                ...
                if (!image.empty()) {
                    ...
                    RCLCPP_DEBUG(this->get_logger(), "IR image published successfully");
                }
            } catch (const std::exception& e) {
    ...
    std::string getLatestImageName() {
        ...
        if (!latest_image.empty()) {
            RCLCPP_INFO(this->get_logger(), "Found latest IR image: %s", latest_image.c_str());
        }
    ...
    void handleCaptureStatus(double* param) {
        if (capture_state_ == CHECK_CAPTURE_STATUS) {
            if (param[0] == 0) {
                ...
                RCLCPP_DEBUG(this->get_logger(), "IR payload is idle, checking camera mode");
            } else {
                capture_state_ = CHECK_STORAGE;
            }
            ...
            if (param[0] == 0) {
                ...
                RCLCPP_DEBUG(this->get_logger(), "IR capture completed, starting download");
            }
        }
    }
    ...
    void handleCameraSettings(double* param) {
        if (capture_state_ == CHECK_CAMERA_MODE) {
            if (param[0] == CAMERA_MODE_IMAGE) {
                ...
                RCLCPP_DEBUG(this->get_logger(), "Camera in IR image mode, starting capture");
            } else {
                capture_state_ = CHANGE_CAMERA_MODE;
                RCLCPP_DEBUG(this->get_logger(), "Changing to IR image mode");
            }
        }
    }
